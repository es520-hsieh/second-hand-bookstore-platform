    ...
    public JSONObject getAll(int id) {
        ...
        try {
            ...
            while(rs.next()) {
                ...
                m = new Member(member_id, email, password, name, status, account, phone, created_at);
    ...
    */
    ...
    public JSONObject create(Member m) {
        ...
        try {
            ...
            conn = DBMgr.getConnection();
            /** SQL指令 */
            String sql = "INSERT INTO `missa`.`member`(`name`, `email`, `password`,`created_at`, `status`, `account`, `phone`)"
                    + " VALUES(?, ?, ?, ?, ?, ?, ?)";
            ...
            pres.setString(6, account);
            pres.setString(7, phone);
    ...
    public JSONObject update(Member m) {
        ...
        try {
            ...
            conn = DBMgr.getConnection();
            /** SQL指令 */
            String sql = "Update `missa`.`member` SET `name` = ? ,`password` = ? ,`phone` = ? ,`email` = ? WHERE `member_id` = ?";
            /** 取得所需之參數 */
            int id = m.getID();
            String name = m.getName();
            String email = m.getEmail();
    ...
    public void updateLoginTimes(Member m) {
        ...
        try {
            ...
            conn = DBMgr.getConnection();
            /** SQL指令
            String sql = "Update `missa`.`member` SET `login_times` = ? WHERE `id` = ?";
            /** 取得會員編號
            int id = m.getID();
	...
	*/
    ...
    public void updateStatus(int id) {      
        ...
        try {
            ...
            conn = DBMgr.getConnection();
            /** SQL指令 */
            String sql = "Update `missa`.`member` SET `status` = ? WHERE `member_id` = ?";
            /** 取得會員編號 */
    ...
    /**更改密碼(for忘記密碼)*/
    public int changePassword(String email, String code) {      
        /** 記錄實際執行之SQL指令 */
        String exexcute_sql = "";
        /** 紀錄SQL總行數，若為「-1」代表資料庫檢索尚未完成 */
        int row = 0;
        ...
        try {
            ...
            /** SQL指令 */
            String sql1 = "Update `missa`.`member` SET `password` = ? WHERE `email` = ?";
            /** 取得會員編號 */
            
            /** 將參數回填至SQL指令當中 */
            pres = conn.prepareStatement(sql1);
            pres.setString(1, code);
            pres.setString(2, email);
            /** 執行更新之SQL指令 */
            pres.executeUpdate();
    ...
    public JSONObject getBargainMemberNameById(int id) {
        /** 新建一個 Member 物件之 m 變數，用於紀錄每一位查詢回之會員資料 */
        Member m = null;
        /** 用於儲存所有檢索回之會員，以JSONArray方式儲存 */
        JSONArray jsa = new JSONArray();
        ...
        try {
            ...
            conn = DBMgr.getConnection();
            /** SQL指令 */
            String sql = "SELECT `name` FROM `missa`.`member` WHERE `member_id` = ? LIMIT 1";
     ...
     * @return the JSONObject 回傳SQL執行結果與自資料庫取回之所有資料
     */
    public JSONObject getUnverifiedUser() {
        /** 新建一個 Member 物件之 m 變數，用於紀錄每一位查詢回之會員資料 */
        Member m = null;
        /** 用於儲存所有檢索回之會員，以JSONArray方式儲存 */
        JSONArray jsa = new JSONArray();
        JSONArray photoName = new JSONArray();
        /** 記錄實際執行之SQL指令 */
        String exexcute_sql = "";
        /** 紀錄程式開始執行時間 */
        long start_time = System.nanoTime();
        ...
        try {
            ...
            conn = DBMgr.getConnection();
            /** SQL指令 */
            String sql = "SELECT * FROM (SELECT * FROM `missa`.`member` WHERE `member`.`status` = 'n') `tmp1` INNER JOIN ( SELECT `member_id`, `imgName` FROM `missa`.`photo` WHERE `member_id` IS NOT NULL ) `tmp2` USING (`member_id`)";
        ...
        response.put("time", duration);
        response.put("data", jsa);
        response.put("photo", photoName);

        return response;
    }
    public boolean checkUpdateDuplicate(Member m){
        /** 紀錄SQL總行數，若為「-1」代表資料庫檢索尚未完成 */
        int row = -1;
        /** 儲存JDBC檢索資料庫後回傳之結果，以 pointer 方式移動到下一筆資料 */
        ResultSet rs = null;
        ...
        try {
            ...
            conn = DBMgr.getConnection();
            /** SQL指令 */
            String sql = "SELECT `member_id` FROM `missa`.`member` WHERE `email` = ? OR `phone`  = ?";
