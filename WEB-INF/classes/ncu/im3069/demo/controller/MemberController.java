...
public class MemberController extends HttpServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        /** 建立一個新的會員物件 */
        Member m = new Member(email, password, name, account, phone);
        
        /** 後端檢查是否有欄位為空值，若有則回傳錯誤訊息 */
        if(email.isEmpty() || password.isEmpty() || name.isEmpty() || account.isEmpty() || phone.isEmpty()) {
            /** 以字串組出JSON格式之資料 */
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (!id.isEmpty()) {
        	...
        	JSONObject query = mh.getUnverifiedUser();
            ...
            JSONObject query = mh.getAll(Integer.parseInt(id));
    ...
    public void doPut(HttpServletRequest request, HttpServletResponse response)
        ...
        Member m = new Member(id, email, password, name, account, phone);
        ...
        /** 透過MemberHelper物件的checkDuplicate()檢查該會員電子郵件信箱是否有重複 */
        else if (mh.checkUpdateDuplicate(m)) {
            ...
            /** 以字串組出JSON格式之資料 */
            String resp = "{\"status\": \'400\', \"message\": \'新增帳號失敗，此E-Mail帳號或手機號碼重複！\', \'response\': \'\'}";
            /** 透過JsonReader物件回傳到前端（以字串方式） */
            jsr.response(resp, response);
        }
